...
protected:
    ...
    struct TreeNode {
        ...
        TreeNode(T const& e, Link i = nullptr, Link d = nullptr,
            int alt = 1) : elem(e), iz(i), dr(d), altura(alt), tam_i(alt) {}
    };
...
public:
    ...
    // Función del ejercicio 1.2 kesimo primer elemento
    // Esta función divide a la mitad el numero de elementos a explorar en cada llamada recursiva por lo que es de coste O(log n)
...
protected:
    ...
    T const& findK(int k, Link a) {
        if (a->tam_i == k)
            return a->elem;
        else if (a->tam_i > k && a->iz != nullptr)
            return findK(k, a->iz);
        else if (a->tam_i < k && a->dr != nullptr)
            return findK(k - a->tam_i, a->dr);
    ...
    bool inserta(T const& e, Link& a) {
        ...
        if (a == nullptr) { // se inserta el nuevo elemento e
            ...
            ++nelems;
            crece = true;
            a->tam_i = 1; // se inicializa el tamaño a uno
        }
        else if (menor(e, a->elem)) {
            ...
            // No se añade nada por la izquierda por lo que no sumamos a tam_i
    ...
    void rotaDer(Link& r2) {
        Link r1 = r2->iz;
        r2->tam_i -= r1->tam_i;  // Actualizamos r1, r2 no cambia
        r2->iz = r1->dr;
        r1->dr = r2;
    ...
    void rotaIzq(Link& r1) {
        Link r2 = r1->dr;
        r2->tam_i += r1->tam_i;  // Actualizamos r2, r1 no cambia
        r1->dr = r2->iz;
        r2->iz = r1;
        r1->altura = std::max(altura(r1->iz), altura(r1->dr)) + 1;
        r2->altura = std::max(altura(r2->iz), altura(r2->dr)) + 1;
